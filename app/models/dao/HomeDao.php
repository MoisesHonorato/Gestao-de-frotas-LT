...
class HomeDao extends Model
{
    ...
    public function contratovenc()
    {
        $sql = "(SELECT  c.*, f.*, p.*, v.id_contrato, SUM(v.vigencia) AS vigencia,
                ...
                ORDER BY restam LIMIT 5";

        return Model::select($this->db, $sql, TRUE);
    }

    public function cnhvenc()
    {
        $sql = "SELECT
                        con.*, co.nome AS condutor, pro.*,
                        DATEDIFF(con.vencimento, CURRENT_DATE) AS restam
                FROM
                        condutor AS con,
                        colaborador AS co,
                        projeto AS pro
                WHERE
                        con.id_motorista = co.id_colaborador
                AND
                        co.id_projeto = pro.id_projeto
                ORDER BY con.vencimento
                LIMIT 5";
